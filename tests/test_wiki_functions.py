...
def test_wikidata2wikipedia():
    ...
    assert process_wikipedia.title_to_id("new_york_city", lower=True, path_to_db=db) == "Q60"
    assert process_wikipedia.title_to_id("new%20york%20city", lower=True, path_to_db=db) == None
    prepare_url = process_wikipedia.make_wikipedia2wikidata_consisent("New York City")
    assert process_wikipedia.title_to_id(prepare_url, lower=True, path_to_db=db) == "Q60"
